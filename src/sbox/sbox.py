class Sbox:
    ...
    def __init__(self, bits: int, table: list[int] | None = None):
        ...
        def _check_table(table: list[int]) -> bool:
            power_of_two_size = _is_power_of_two(len(table))
            return power_of_two_size and _is_permutation(table)
    ...
    def algebraic_degree(self) -> int:
        def _pascal_transform(
            function: list[int], left: int = 0, right: int | None = None
        ):
            if right is None:
                right = len(function)
            size = right - left
